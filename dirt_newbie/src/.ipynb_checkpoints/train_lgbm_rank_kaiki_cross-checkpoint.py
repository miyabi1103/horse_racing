...
class Trainer_lightgbm_rank_kaiki_cv:
    def __init__(
        self, 
        input_dir: Path = INPUT_DIR, 
        features_filename: str = "features.csv", 
    ...
    def create_dataset_for_cv(
        ...
        """
        # 目的変数
        self.features["target"] = self.features["rank"].astype(int)
        ...
        for i in range(n_splits):
            ...
            valid_df = self.features.iloc[(i + 1) * fold_size: (i + 2) * fold_size]
            # 「馬の過去成績」の欠損率
            valid_df = valid_df.query('rank_3races.isna()')
            # 学習データと評価データを返す
            yield train_df, valid_df
    ...
    def run(
        self, 
        n_splits: int = 10, 
        importance_filename: str = "importance_lightgbm_rank_kaiki_cv", 
        model_filename: str = "model_lightgbm_rank_kaiki_cv.pkl", 
        evaluation_filename: str = "evaluation_lightgbm_rank_kaiki_cv.csv", 
        final_model_filename: str = "model_lightgbm_rank_kaiki_cv_full.pkl"
    ):
