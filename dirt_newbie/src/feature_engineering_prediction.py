...
class PredictionFeatureCreator:
    ...
    def fetch_race_info(
        ...
        # まず、型を推測して明示的に設定する
        df["place_course_category"] = df["place_course_category"].infer_objects()
        #-1を欠損値として扱いintに直す
        ...
        # さらに他の列にも適用
        df[['goal_range', 'curve', 'goal_slope']] = df[['goal_range', 'curve', 'goal_slope']].infer_objects()
    ...
    def create_features(
        ...
        features["goal_range"] = features["goal_range"].fillna(-1)
        features["goal_range_100"] = features["goal_range_100"].fillna(-1)
