...
def create(
    ...
    results["age"] = results["性齢"].str[1:].astype(int)
    # 1. race_idごとの年齢の平均、中央値、平均（小数点以下切り捨て）を計算
    race_age_stats = results.groupby('race_id').agg(
        mean_age=('age', 'mean'),
    ...
    results = results.loc[:, ~results.columns.str.endswith('_drop')]


    population = (
        race_info.query("@from_ <= date <= @to_")[["race_id", "date"]]
        .merge(results.query("3 == mean_age")[["race_id", "horse_id","mean_age"]], on="race_id")
        .sort_values(["date", "race_id"])
    )
    population.drop(columns=["mean_age"], inplace=True)
