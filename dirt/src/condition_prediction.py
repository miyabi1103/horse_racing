...
RAWDF_DIR = COMMON_DATA_DIR / "rawdf2"
...
def process_results(
    ...
    output_filename: str = "results_prediction.csv",
...
def process_race_info(
    ...
    output_filename: str = "race_info_prediction.csv",
    ...
    #-1を欠損値として扱いintに直す
    df["place_course_category"] = df["place_course_category"].fillna(-10000).astype(int)
    ...
    # 新しい列を追加
    df["place_course_tough"] = df["place_course_category"].map(conversion_map).fillna(-10000).astype(int)
...
def create_race_grade(    
    ...
    output_dir: Path = OUTPUT_DIR,
    input_filename_results: str = "results_prediction.csv",
    sex_mapping: dict = sex_mapping,
    population_dir: Path = POPULATION_DIR,
    populaton_filename: str = "population.csv",
    input_filename_info: str = "race_info_prediction.csv",
    output_filename: str = "race_info_prediction.csv",
    race_type_mapping: dict = race_type_mapping,
    around_mapping: dict = around_mapping,
    weather_mapping: dict = weather_mapping,
    ...
    results_df = pd.read_csv(input_dir / input_filename_results , sep="\t")
    info_df = pd.read_csv(input_dir / input_filename_info, sep="\t")
    df = (
        results_df[["race_id", "horse_id","mean_age_kirisute"]]
        .merge(info_df[["race_id", "race_type", "season","race_class"]], on="race_id")
    )
    ...
    info_df[['age_season', 'race_grade', 'race_grade_scaled']] = df[['age_season', 'race_grade', 'race_grade_scaled']]
    info_df.to_csv(output_dir/output_filename, sep="\t", index=False)
    return info_df
