...
class Trainer_lightgbm_rank_niti_cv_in3:
    def __init__(
        self, 
        input_dir: Path = INPUT_DIR, 
        features_filename: str = "features.csv", 
    ...
    def create_dataset_for_cv(
        ...
        """
        # 目的変数
        self.features["target"] = (self.features["rank"] < 4).astype(int)
    ...
    def train(
        ...
        # 評価
        evaluation_df = valid_df[["race_id", "horse_id", "target", "rank","course_len","weather","ground_state","race_class","age","median_age","mean_age","place", "tansho_odds", "popularity","n_horses", "umaban"]].copy()
        evaluation_df["pred"] = model.predict(valid_df[self.feature_cols], num_iteration=model.best_iteration)
