...
async def Auto_purchase_wide(race_id:str, top_n: int = 2,amount: str = "100",amount_num: str = "1"):
    ...
    async with async_playwright() as playwright:
        ...
        try:
            ...
            print(f"普通クリック失敗: {e}。ワイドを探します。")
            try:
                # ボタン全部取得
                buttons = await page1.locator("td:nth-child(4) > .btn-mark").all()
                ...
                if not buttons:
                    ...
                    if not clicked:
                        ...
                        print("ワイドが見つからなかったので最初のボタンをクリックします")
                        await buttons[0].click()
                        await asyncio.sleep(2)  # ★ クリック後に1秒待機

            except Exception as e2:
                print(f"ワイド探しも最初のクリックも失敗しました: {e2}")
                # ここで諦めてスキップ
