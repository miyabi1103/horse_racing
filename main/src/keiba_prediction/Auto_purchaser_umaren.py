...
async def Auto_purchase_umaren(race_id:str, top_n: int =2,amount: str = "100",amount_num: str = "1"):
    ...
    async with async_playwright() as playwright:
        ...
        # await page1.locator("td:nth-child(2) > .btn-mark").click()
        ...
        try:
            ...
            print(f"普通クリック失敗: {e}。馬連を探します。")
            try:
                # ボタン全部取得
                buttons = await page1.locator("td:nth-child(2) > .btn-mark").all()
                ...
                if not buttons:
                    ...
                    if not clicked:
                        ...
                        print("馬連が見つからなかったので最初のボタンをクリックします")
                        await buttons[0].click()
                        await asyncio.sleep(2)  # ★ クリック後に1秒待機

            except Exception as e2:
                print(f"馬連探しも最初のクリックも失敗しました: {e2}")
                # ここで諦めてスキップ
        ...
        if not clicked:
            try:
                btn = page1.get_by_role("button", name=first_umaban)
        ...
        # パターン①: セルの中のボタン
        try:
            cell = page1.get_by_role("cell", name=second_umaban, exact=True)
            if await cell.is_visible():
                await cell.get_by_role("button").click()
                clicked = True  # 成功したらフラグを立てる
        except:
        ...
        if not clicked:
            try:
                btn = page1.get_by_role("button", name=second_umaban)
                if await btn.is_visible():
                    await btn.click()
                    clicked = True
            except:
        ...
        # await page1.get_by_role("cell", name="合計金額入力： 円 金額セット用テンキ―").get_by_role("button").click()
        # await page1.locator("#bet-list-top").get_by_role("button", name="1").click()
        # await page1.get_by_role("button", name="0", exact=True).click()
        # await page1.get_by_role("button", name="0", exact=True).dblclick()
        # await page1.locator("#bet-list-top").get_by_role("button", name="セット", exact=True).click()
...
if __name__ == "__main__":
    asyncio.run(Auto_purchase_umaren())
