...
def process_results(
    ...
    df["weight"] = df["馬体重"].str.extract(r"(\d+)").fillna(400).astype(int)
...
def process_race_info(
    ...
    # 正規表現パターンを作成
    regex_race_class_detail = "|".join(list(race_class_mapping.keys()))
    regex_race_class_general = "|".join(list(race_class_mapping_general.keys()))

    # タイトルから詳細階級を抽出
    df["race_class_detail"] = (
        df["title"]
        .str.extract(rf"({regex_race_class_detail})")[0] # 詳細階級を抽出
        .fillna(df["info2"].str.extract(rf"({regex_race_class_detail})")[0])  # info2から抽出
    )

    # 詳細階級が見つかった場合、それを使い、なければ汎用階級を使用
    df["race_class"] = (
        df["race_class_detail"]
        .map(lambda x: race_class_mapping.get(x) if pd.notna(x) else None)  # 詳細階級のマッピング
        .fillna(
            df["title"]
            .str.extract(rf"({regex_race_class_general})")[0] # 汎用階級をタイトルから抽出
            .fillna(df["info2"].str.extract(rf"({regex_race_class_general})")[0])  # info2から抽出
            .map(lambda x: race_class_mapping_general.get(x))  # 汎用階級のマッピング
        )
    )
...
def process_horse_results(
    ...
    ダート重賞（３歳を除く）	－	１００（２０）
    古馬Ｇ１	９８（１８）	１０５（２５）
    大井＝船橋≧川崎＞門別＞高知≧園田=姫路≧浦和≧名古屋＞盛岡＞水沢＞金沢＞＞佐賀＞笠松
    """
    place_adjustment = {
        44: 0, 43: 0, 45: -1,30: -3,
