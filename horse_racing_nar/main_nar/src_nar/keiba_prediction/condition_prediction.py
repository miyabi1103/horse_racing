...
POPULATION_DIR = Path("..","..", "data_nar", "00_population")
OUTPUT_DIR = Path("..","..", "data_nar", "01_preprocessed")
...
def process_race_info(
    ...
    df["race_type"] = df["race_type"].fillna(0).astype(int).astype(str)
    ...
    df["ground_state"] = df["ground_state"].replace([np.inf, -np.inf], np.nan)  # inf を NaN に変換
    df["ground_state"] = df["ground_state"].fillna(0)  # NaN を 0 に置き換え
    ...
    df["race_class"] = df["race_class"].replace([np.inf, -np.inf], np.nan)  # inf を NaN に変換
    df["race_class"] = df["race_class"].fillna(-13)  # NaN を 0 に置き換え
...
def create_race_grade(    
    ...
    df = (
        ...
        .merge(info_df[["race_id", "race_type","season_level","race_class"]], on="race_id")
    )
    race_i = info_df
    ...
    df_agg = df.groupby("race_id", as_index=False).agg({"race_grade": "first", "race_grade_scaled": "first"})
    info_df = (
        race_i
        .merge(df_agg[["race_id",'race_grade', 'race_grade_scaled']], on="race_id",how="left")
    )
    info_df.dropna(subset=['race_grade'], inplace=True)
