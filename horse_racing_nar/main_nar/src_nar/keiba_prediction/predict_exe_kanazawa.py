...
def def_predict_exe_kanazawa(kaisai_date:str,race_id:str):
    pfc_dirt = PredictionFeatureCreator(
        ...
        old_population_filename="population_all.csv",
        old_results_filename="results_all.csv",
        old_race_info_filename="race_info_all.csv",
        old_horse_results_filename="horse_results_all.csv",
    )
    # 過去成績集計は事前に行うことができる
    ...
    prediction.predict(
        features,
        model_filename="model_lightgbm_rank_niti_cv_full_dev_turf_only_tenkai_weight_kanazawa_in3.pkl",
        config_filepath="config_lightgbm_niti_dev_dirt_only_tenkai_weight4.yaml"
    )
