...
OUTPUT_INFO_DIR = Path("..", "data_nar", "01_preprocessed")
...
class FeatureCreator:
    ...
    def cross_features_3(
        ...
        df = (
            baselog
            .merge(self.all_results[["race_id", "horse_id", "wakuban","rank", "nobori","time","umaban"]], on=["race_id", "horse_id"])
        )
        ...
        df = df.copy()   
        df = df[df['rank'].isin([1, 2, 3,4,5,6,7])]          # ターゲットエンコーディングを計算（カテゴリごとのtimeの平均）
        df = df.copy()           
    ...
    def cross_features_17(
        self
            ...
            def compute_mean_for_row(row, df_old2_1):
                ...
                # 5. 計算結果を返す（NaNの場合も考慮）
                # return mean_time_diff if not np.isnan(mean_time_diff) else filtered_df_old2_2["distance_place_type_race_grade_encoded_time_diff"].mean()
                return np.nan_to_num(mean_time_diff, nan=np.nan_to_num(filtered_df_old2_2["distance_place_type_race_grade_encoded_time_diff"].mean(), nan=0.5))
