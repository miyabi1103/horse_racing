    ...
    for html_path in tqdm(html_path_list):
        ...
        with open(html_path, "rb") as f:
            try:
                ...
                df_list_pay1 = pd.read_html(html, attrs={'class': "pay_table_01"})

                # テーブルが存在しない場合、警告メッセージを表示して次へ
                if not df_list_pay1:
                    print(f"No table found for race_id {race_id}. Skipping this file.")
                    continue                
                    # print(df_list[0])
                # print(df_list[1])
                # print(df_list[2])
                ...
                # print(df_list_pay1)
                # テーブルが複数存在する場合、2つのテーブルを結合
                if len(df_list_pay1) > 1:
                    df = pd.concat([df_list_pay1[0], df_list_pay1[1]])
                else:
                    df = df_list_pay1[0]  # 1つのテーブルのみの場合

                # 最初の列にrace_idを挿入
                df.insert(0, "race_id", race_id)
                dfs[race_id] = df
            except IndexError as e:
                print(f"IndexError: Table not found in race_id {race_id}. Skipping this file.")
                continue
            except Exception as e:
                print(f"Unexpected error for race_id {race_id}: {e}. Skipping this file.")
                continue
    concat_df = pd.concat(dfs.values())
    save_dir.mkdir(exist_ok=True, parents=True)
    update_rawdf(
...
#     for html_path in tqdm(html_path_list):
#         try:
#             with open(html_path, "r", encoding="euc-jp", errors="ignore") as f:
#                 html = f.read()

#             # BeautifulSoup で `<table>` の数を確認
#             soup = BeautifulSoup(html, "lxml")
#             tables = soup.find_all("table")
#             print(f"Processing race_id: {html_path.stem}, Found {len(tables)} tables")

#             if len(tables) == 0:
#                 print(f"Skipping {html_path.stem}: No tables found in HTML")
#                 continue

#             # `pd.read_html()` の結果を確認
#             df_list = pd.read_html(StringIO(html))
#             print(f"race_id: {html_path.stem}, df_list length: {len(df_list)}")

#             if not df_list:
#                 print(f"Skipping {html_path.stem}: No valid tables found")
#                 continue

#             # 各テーブルの形を出力
#             for i, df in enumerate(df_list):
#                 print(f"race_id: {html_path.stem}, Table {i}: Shape {df.shape}")
#                 print(df.head())  # 最初の数行を表示して確認

#             # `df_list` の長さをチェック
#             if len(df_list) < 3:
#                 print(f"Warning: {html_path.stem} has only {len(df_list)} tables, expected at least 3.")
#                 continue  # 必要なデータが揃ってないのでスキップ
...
#         except Exception as e:
#             print(f"Error processing {html_path.stem}: {e}")
#             continue
