...
v3_0_5は便宜上、開発時に使用していたフォルダです、v3_0_5が最新で、その後、siba/dirt/obstaclesフォルダにそれぞれ名前が置き換わっています
siba/dirt/obstaclesはそれぞれ、芝、ダート、障害を予想する用で分けられています
_sinbaはそれぞれのタイプで新馬や過去レースが少ない馬が走る場合を見越した予測版です、学習時に過去戦績がない馬に最適化するようにしています
...
スクレイピングが終わっている場合
siba/dirt/obstaclesフォルダにあるいずれかのsrc/main2を上から実行すること
...
commonフォルダのsrc/main2の下の方にある、事前情報集計を実行（該当レースの日時をnetkeibaのurlから手動で取得してきてください、サンプルを参考に手動で入れても良いです、サンプルは2024年エリザベス女王杯の日時、race_idです）
siba/dirt/obstaclesフォルダにあるいずれかのsrc/main2の下の方にある、予測処理を行うことで予測が可能（該当レースのrace_idをnetkeibaのurlから手動で取得、サンプルは2024エリザベス女王杯を予測しようとした時のもの）
